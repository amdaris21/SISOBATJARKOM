...
<body class="antialiased text-[#4E342E] min-h-screen flex flex-col">
    ...
    <main class="flex-grow w-full pb-20">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12">
            ...
            <form method="GET" action="{{ route('lesson') }}" class="w-full max-w-[800px] h-[60px] bg-white border border-[#B8B8B8] shadow-sm rounded-full mx-auto mt-12 flex items-center px-6 gap-4">
                @if(request('category'))
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari materi (Contoh: Dasar jaringan, topologi)" class="w-full bg-transparent border-none outline-none font-medium text-[15px] text-[#AFABAA] placeholder-[#AFABAA] focus:ring-0">
                <button type="submit" class="hidden">Cari</button>
            </form>

            <div class="flex flex-wrap items-center justify-center gap-4 md:gap-6 mt-16">
                @php
                    $currentCat = strtolower(request('category', 'semua'));
                    $activeClass = 'bg-[#B7131A] text-white';
                    $inactiveClass = 'bg-[#FFCCCF] text-white hover:bg-[#B7131A]';
                @endphp
                <a href="{{ route('lesson', ['category' => 'Semua', 'search' => request('search')]) }}" class="px-8 py-2.5 border border-[#B8B8B8] rounded-[15px] text-[18px] md:text-[20px] font-normal shadow-sm transition {{ $currentCat === 'semua' || empty($currentCat) ? $activeClass : $inactiveClass }}">Semua</a>
                <a href="{{ route('lesson', ['category' => 'Dasar', 'search' => request('search')]) }}" class="px-8 py-2.5 border border-[#B8B8B8] rounded-[15px] text-[18px] md:text-[20px] font-normal shadow-sm transition {{ $currentCat === 'dasar' ? $activeClass : $inactiveClass }}">Dasar</a>
                <a href="{{ route('lesson', ['category' => 'Topologi', 'search' => request('search')]) }}" class="px-8 py-2.5 border border-[#B8B8B8] rounded-[15px] text-[18px] md:text-[20px] font-normal shadow-sm transition {{ $currentCat === 'topologi' ? $activeClass : $inactiveClass }}">Topologi</a>
                <a href="{{ route('lesson', ['category' => 'Teori', 'search' => request('search')]) }}" class="px-8 py-2.5 border border-[#B8B8B8] rounded-[15px] text-[18px] md:text-[20px] font-normal shadow-sm transition {{ $currentCat === 'teori' ? $activeClass : $inactiveClass }}">Teori</a>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-16">
                @forelse ($lessons as $index => $lesson)
                    @php
                        $levelBg = ['Easy' => 'bg-[#D1F7DB]', 'Medium' => 'bg-[#FEEED1]', 'Hard' => 'bg-[#FFD9D9]'][$lesson->level ?? 'Easy'] ?? 'bg-[#D1F7DB]';
                        $levelText = ['Easy' => 'text-[#2DC36A]', 'Medium' => 'text-[#F39C12]', 'Hard' => 'text-[#E74C3C]'][$lesson->level ?? 'Easy'] ?? 'text-[#2DC36A]';
                        
                        $iconColors = ['bg-[#0e76ec] bg-opacity-80', 'bg-[#CA58E7]', 'bg-[#D9C1BC]', 'bg-[#DB6369]', 'bg-[#6AB1F7]', 'bg-[#FFB300]', 'bg-[#E58735]', 'bg-[#72D3F6]', 'bg-[#5461EB]'];
                        $icons = ['🌐', '📑', '☁️', '🖧', '🖥️', '🛡️', '▶️', '⚙️', '🌐'];

                        $iconColor = $iconColors[$index % count($iconColors)];
                        $icon = $icons[$index % count($icons)];
                        
                        $isCompleted = (Auth::check() && $lesson->progress && $lesson->progress->count() > 0);
                        $progressPercent = $isCompleted ? 100 : 0;
                    @endphp

                    <div class="w-full bg-white border border-[#B8B8B8] rounded-[15px] p-6 flex flex-col justify-between relative shadow-sm hover:-translate-y-1 transition-transform group min-h-[240px]">
                        <div>
                            <div class="flex items-center gap-4 mb-4">
                                <div class="w-12 h-12 {{ $iconColor }} rounded-[12px] flex items-center justify-center shrink-0">
                                    <span class="text-2xl text-white">{{ $icon }}</span>
                                </div>
                                <h3 class="text-[16px] font-semibold text-black leading-tight">{{ $lesson->title }}</h3>
                            </div>
                            <p class="text-[13px] text-[#A5A5A5] mb-6 leading-relaxed font-medium">{{ \Illuminate\Support\Str::limit($lesson->description, 85) }}</p>
                        </div>
                        
                        <div class="mt-auto flex flex-col gap-4">
                            <div class="flex items-center justify-between">
                                <div class="px-5 py-1.5 {{ $levelBg }} rounded-full flex items-center justify-center">
                                    <span class="font-bold text-[12px] {{ $levelText }} tracking-wide">{{ $lesson->level ?? 'Easy' }}</span>
                                </div>
                                <a href="{{ route('lesson.show', $lesson->slug) }}" class="px-5 py-1.5 bg-white border border-[#B8B8B8] rounded-full flex items-center justify-center gap-2 hover:bg-gray-50 transition shadow-sm">
                                    <span class="text-[12px] font-semibold text-black">{{ $isCompleted ? 'Selesai' : 'Pelajari' }}</span>
                                    <span class="text-[14px] font-bold text-black">&rsaquo;</span>
                                </a>
                            </div>
                            <div class="w-full relative h-[10px] rounded-full bg-gray-100 border border-[#E5E5E5] overflow-hidden">
                                <div class="h-full {{ $isCompleted ? 'bg-[#30D46F]' : 'bg-[#DC7175]' }} rounded-full" style="width: {{ $progressPercent }}%;"></div>
                            </div>
                        </div>
                    </div>
                @empty
                    <div class="col-span-full py-12 text-center">
                        <p class="text-gray-500">Materi belum tersedia saat ini.</p>
                    </div>
                @endforelse
