    ...
    <header class="w-full bg-white border-b border-[#B8B8B8] sticky top-0 z-50 h-[88px] flex items-center">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full flex items-center justify-between">
            ...
            <nav class="hidden md:flex items-center gap-8">
                ...
                <a href="{{ route('admin.lessons.index') }}" class="text-[15px] font-medium text-black border-b-2 border-[#B7131A] pb-1">Kelola Materi</a>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-12">
            ...
            <div class="w-full bg-white border border-[#B8B8B8] rounded-[15px] flex flex-col md:flex-row items-center justify-between shadow-sm hover:shadow-[0_8px_30px_rgb(0,0,0,0.12)] transition-shadow duration-300 min-h-[250px] md:min-h-[300px] p-6 md:p-10 mb-8">
                ...
                <div class="mt-6 md:mt-0 flex-shrink-0 w-[200px] h-[200px] md:w-[260px] md:h-[260px] mr-0 md:mr-4 lg:mr-8">
                    <img src="{{ asset('images/materi.jpeg') }}" alt="Ilustrasi Materi" class="w-full h-full object-contain mix-blend-multiply hover:scale-105 transition-transform duration-300">
                </div>
            </div>
            ...
            <div class="w-full bg-white border border-[#CAC7C6] rounded-[15px] overflow-hidden shadow-sm">
                ...
                <div class="flex flex-col">
                    ...
                    <div class="px-6 py-4 flex items-center justify-between border-b border-[#AFABAA] hover:bg-red-50 transition-colors group cursor-default">
                        <div class="w-5/12 text-[14px] md:text-[15px] text-black">{{ $lesson->title }}</div>
                        <div class="w-2/12 flex justify-center">
                            <span class="bg-[#C9373F] border border-white text-white font-josefin font-bold text-[12px] px-4 py-1 rounded-[12px]">{{ $lesson->category ?? 'Dasar' }}</span>
                        </div>
                        <div class="w-2/12 flex justify-center">
                            @php
                                $levelColors = [
                                    'Easy' => 'bg-[#CAFCDF] text-[#30D46F]',
                                    'Medium' => 'bg-[#FCE7CA] text-[#D27634]',
                                    'Hard' => 'bg-[#FCCACA] text-[#D23434]'
                                ];
                                $colorClass = $levelColors[$lesson->level ?? 'Easy'] ?? 'bg-[#CAFCDF] text-[#30D46F]';
                            @endphp
                            <span class="{{ $colorClass }} border border-white font-josefin font-bold text-[12px] px-4 py-1 rounded-[12px]">{{ $lesson->level ?? 'Easy' }}</span>
                        </div>
                        <div class="w-3/12 flex justify-center items-center gap-4">
                            <a href="{{ route('admin.lessons.edit', $lesson->id) }}" class="flex flex-col items-center group/btn hover:-translate-y-0.5 transition-transform">
                            ...
                            <form action="{{ route('admin.lessons.destroy', $lesson->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus materi ini?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="flex flex-col items-center group/btn hover:-translate-y-0.5 transition-transform">
                ...
                <div @click.outside="isModalOpen = false" class="bg-white rounded-[10px] w-full max-w-xl p-5 md:p-6 relative max-h-[90vh] overflow-y-auto" style="border: 1px solid #A5A5A5; box-shadow: 0px 4px 4px #B7131A;">
                    ...
                    <form action="{{ route('admin.lessons.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="title" class="w-full bg-white border border-[#B8B8B8] rounded-[8px] h-[38px] md:h-[42px] px-4 text-[13px] md:text-[14px] focus:outline-none focus:border-[#B7131A] shadow-[0px_2px_4px_#E2E2E2]" required>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
                            <div>
                                <label class="block text-[13px] md:text-[14px] text-[#4E342E] mb-1 font-normal">Modul</label>
                                <select name="module_id" class="w-full bg-white border border-[#A5A5A5] rounded-[8px] h-[38px] md:h-[42px] px-4 text-[13px] md:text-[14px] focus:outline-none focus:border-[#B7131A]" required>
                                    @php
                            ...
                            <div>
                                <label class="block text-[13px] md:text-[14px] text-[#4E342E] mb-1 font-normal">Status</label>
                                <select name="status" class="w-full bg-white border border-[#A5A5A5] rounded-[8px] h-[38px] md:h-[42px] px-4 text-[13px] md:text-[14px] focus:outline-none focus:border-[#B7131A]" required>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
                            <div>
                                ...
                                <input type="text" name="category" class="w-full bg-white border border-[#A5A5A5] rounded-[8px] h-[38px] md:h-[42px] px-4 text-[13px] md:text-[14px] focus:outline-none focus:border-[#B7131A] shadow-[0px_2px_4px_#E2E2E2]" required>
                            </div>
                            <div>
                                <label class="block text-[13px] md:text-[14px] text-[#4E342E] mb-1 font-normal">Level</label>
                                <select name="level" class="w-full bg-white border border-[#A5A5A5] rounded-[8px] h-[38px] md:h-[42px] px-4 text-[13px] md:text-[14px] focus:outline-none focus:border-[#B7131A] shadow-[0px_2px_4px_#E2E2E2]" required>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="description" class="w-full bg-white border border-[#A5A5A5] rounded-[8px] h-[80px] md:h-[100px] p-3 text-[13px] md:text-[14px] resize-none focus:outline-none focus:border-[#B7131A] shadow-[0px_2px_4px_#E2E2E2]"></textarea>
                        </div>

                        <!-- Unggah File Baru -->
                        <div class="mb-6">
                            <label class="block text-[13px] md:text-[14px] text-[#4E342E] mb-1 font-normal">Unggah File <span class="text-[#B7131A]">Materi Baru</span></label>
                            <input type="file" name="file" class="w-full bg-white border border-[#B8B8B8] rounded-[8px] h-[38px] md:h-[42px] px-4 pt-[7px] text-[13px] md:text-[14px] focus:outline-none focus:border-[#B7131A] shadow-[0px_2px_4px_#E2E2E2]">
                        </div>
