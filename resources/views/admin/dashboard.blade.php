    ...
    <header class="w-full bg-white border-b border-[#B8B8B8] sticky top-0 z-50 h-[88px] flex items-center">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full flex items-center justify-between">
            ...
            <nav class="hidden md:flex items-center gap-8">
                ...
                <a href="{{ route('admin.quizzes.index') }}" class="text-[15px] font-medium text-gray-700 hover:text-black transition">Kelola Kuis</a>
            </nav>
    ...
    <main class="flex-grow w-full pb-20 bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-12">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                <div class="bg-white border border-[#E0D7D5] rounded-[15px] p-5 shadow-[0_6px_0_0_#D69600] hover:-translate-y-1 hover:shadow-[0_10px_0_0_#D69600] transition-all duration-300 group cursor-pointer">
                    ...
                    <div class="flex items-center justify-between px-10">
                        <div class="text-[32px] font-bold text-[#361F1A] leading-none tracking-tight">{{ number_format($totalUsers ?? 0) }}</div>
                ...
                <div class="bg-white border border-[#E0D7D5] rounded-[15px] p-5 shadow-[0_6px_0_0_#A10C13] hover:-translate-y-1 hover:shadow-[0_10px_0_0_#A10C13] transition-all duration-300 group cursor-pointer">
                    ...
                    <div class="flex items-center justify-between px-10">
                        <div class="text-[32px] font-bold text-[#361F1A] leading-none tracking-tight">{{ number_format($totalLessons ?? 0) }}</div>
                ...
                <div class="bg-white border border-[#E0D7D5] rounded-[15px] p-5 shadow-[0_6px_0_0_#4E342E] hover:-translate-y-1 hover:shadow-[0_10px_0_0_#4E342E] transition-all duration-300 group cursor-pointer">
                    ...
                    <div class="flex items-center justify-between px-10">
                        <div class="text-[32px] font-bold text-[#361F1A] leading-none tracking-tight">{{ number_format($totalQuizzes ?? 0) }}</div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-20">
                <div class="bg-white border border-[#E0D7D5] rounded-[15px] p-6 shadow-[4px_4px_10px_rgba(0,0,0,0.05)] hover:-translate-y-1 hover:shadow-[4px_4px_15px_rgba(0,0,0,0.1)] transition-all duration-300">
                    <div class="flex justify-between items-start mb-10">
                        <div>
                            ...
                            <p class="text-[14px] text-[#504442] mt-1">Aktivitas pembelajaran selama {{ $range }} hari terakhir</p>
                        </div>
                        <div x-data="{ open: false }" class="relative z-20">
                            <button @click="open = !open" class="px-3 py-1.5 bg-[#FFF8F6] border border-[#D4C3BF] rounded-[8px] flex items-center gap-2 hover:bg-[#ffece6] transition-colors focus:outline-none">
                                <span class="text-[14px] text-[#1E1B1A]">{{ $range }} Hari Terakhir</span>
                                <svg class="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                            </button>
                            <div x-show="open" @click.outside="open = false" style="display: none;" class="absolute right-0 mt-2 w-40 bg-white border border-[#B8B8B8] shadow-lg rounded-[10px] py-1">
                                <a href="?range=7" class="block px-4 py-2 text-[14px] text-gray-700 hover:bg-gray-50">7 Hari Terakhir</a>
                                <a href="?range=9" class="block px-4 py-2 text-[14px] text-gray-700 hover:bg-gray-50">9 Hari Terakhir</a>
                                <a href="?range=14" class="block px-4 py-2 text-[14px] text-gray-700 hover:bg-gray-50">14 Hari Terakhir</a>
                                <a href="?range=30" class="block px-4 py-2 text-[14px] text-gray-700 hover:bg-gray-50">30 Hari Terakhir</a>
                            </div>
                        </div>
                    </div>
                    <div class="h-[250px] w-full flex items-end justify-between px-2 pt-6 gap-1 md:gap-2">
                        @foreach($dailyActivity as $index => $height)
                        <div class="flex-1 h-[{{ $height }}%] {{ $height > 50 ? 'bg-[#B7131A]' : 'bg-[rgba(255,218,214,0.3)]' }} rounded-t-[4px] relative group cursor-pointer border border-[#B7131A] hover:bg-[#91000A] transition-colors duration-300">
                             <div class="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-[#361F1A] text-white text-[10px] px-2 py-1 rounded-[4px] opacity-0 group-hover:opacity-100 transition-all z-10 whitespace-nowrap">
                                {{ $dailyCounts[$index] }} Aktivitas
                             </div>
                        </div>
                        @endforeach
                    </div>
                    <div class="flex justify-between mt-4 px-2 text-[10px] font-bold text-[#504442]">
                        <span>{{ $range }} HARI LALU</span>
                        <span>HARI INI</span>
                    </div>
                </div>
                ...
                <div class="bg-white border border-[#E0D7D5] rounded-[15px] p-6 shadow-[4px_4px_10px_rgba(0,0,0,0.05)] hover:-translate-y-1 hover:shadow-[4px_4px_15px_rgba(0,0,0,0.1)] transition-all duration-300 flex flex-col justify-between">
                    ...
                    <div class="flex flex-col gap-6">
                        @forelse($popularLessons as $lesson)
                        @php
                            $percentage = $totalUsers > 0 ? round(($lesson->progress_count / $totalUsers) * 100) : 0;
                            $colors = ['bg-[#B7131A]', 'bg-[#FFBA38]', 'bg-[#16A34A]', 'bg-[#3b82f6]'];
                            $colorClass = $colors[$loop->index % 4];
                            $textColors = ['text-[#B7131A]', 'text-[#FFBA38]', 'text-[#16A34A]', 'text-[#3b82f6]'];
                            $textColorClass = $textColors[$loop->index % 4];
                        @endphp
                        <div>
                            <div class="flex justify-between items-center mb-2">
                                <span class="text-[14px] font-semibold text-[#361F1A]">{{ $lesson->title }}</span>
                                <span class="text-[14px] font-semibold {{ $textColorClass }}">{{ $percentage }}% Selesai</span>
                            </div>
                            <div class="w-full h-[12px] bg-[#F4ECEA] rounded-full overflow-hidden relative">
                                <div class="absolute left-0 top-0 bottom-0 {{ $colorClass }} rounded-full" style="width: {{ $percentage }}%"></div>
                            </div>
                        </div>
                        @empty
                        <div class="text-[14px] text-[#7B7675]">Belum ada data materi yang diselesaikan.</div>
                        @endforelse
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-20">
                <a href="{{ route('admin.lessons.index') }}" class="bg-white border border-[#E0D7D5] rounded-[15px] p-5 shadow-[4px_4px_10px_rgba(0,0,0,0.05)] flex items-center justify-between group hover:-translate-y-1 hover:shadow-[4px_4px_15px_rgba(0,0,0,0.1)] transition-all duration-300">
                ...
                <a href="{{ route('admin.quizzes.index') }}" class="bg-white border border-[#E0D7D5] rounded-[15px] p-5 shadow-[4px_4px_10px_rgba(0,0,0,0.05)] flex items-center justify-between group hover:-translate-y-1 hover:shadow-[4px_4px_15px_rgba(0,0,0,0.1)] transition-all duration-300">
                    <div class="flex items-center gap-5">
                        <div class="w-[60px] h-[60px] flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
