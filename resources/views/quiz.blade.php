...
<body class="antialiased text-[#4E342E] min-h-screen flex flex-col">
    ...
    <!-- Main Content -->
    <main class="flex-grow w-full pb-20 relative" x-data="{ 
        showModal: false, 
        quizId: null,
        quizTitle: '', 
        quizLevel: '', 
        quizQuestions: 10, 
        zepLink: 'https://zep.us/',
        isStarted: false,
        
        startQuiz() {
            this.isStarted = true;
            fetch('/quiz/' + this.quizId + '/start', {
                method: 'POST',
                headers: {
            ...
            window.open(this.zepLink, '_blank');
        },

        completeQuiz() {
            fetch('/quiz/' + this.quizId + '/complete', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10">
            ...
            <div class="flex flex-col md:flex-row items-center justify-between gap-6 mb-10">
                <!-- Search bar -->
                <form method="GET" action="{{ route('quiz') }}" class="relative w-full md:w-[460px]">
                    <span class="absolute inset-y-0 left-0 flex items-center pl-4 text-gray-400">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                    </span>
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari quiz (Contoh: OSI, Jaringan)" class="w-full pl-12 pr-4 py-3 bg-white border border-[#B8B8B8] rounded-[10px] text-sm text-black placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-[#B7131A] shadow-[inset_0_1px_2px_rgba(0,0,0,0.05)]">
                    <button type="submit" class="hidden">Cari</button>
                </form>

                <!-- Stats widgets -->
                <div class="flex gap-4 w-full md:w-auto justify-end">
                    <!-- Rata-rata Skor Dihapus Sesuai Permintaan -->
                    ...
                    <div class="bg-white border border-[#B8B8B8] rounded-[10px] px-4 py-2 flex items-center gap-3 shadow-[4px_4px_10px_rgba(0,0,0,0.02)] min-w-[170px]">
                        ...
                        <div>
                            ...
                            <div class="text-[14px] font-bold text-black mt-1">{{ $completedQuizCount ?? 0 }}/{{ $quizzes->count() }}</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                @forelse ($quizzes as $index => $quiz)
                    @php
                        $levelBg = ['Easy' => 'bg-[#D1F7DB]', 'Medium' => 'bg-[#FEEED1]', 'Hard' => 'bg-[#FFD9D9]'][$quiz->level ?? 'Easy'] ?? 'bg-[#D1F7DB]';
                        $levelText = ['Easy' => 'text-[#2DC36A]', 'Medium' => 'text-[#F39C12]', 'Hard' => 'text-[#E74C3C]'][$quiz->level ?? 'Easy'] ?? 'text-[#2DC36A]';
                        $iconBg = 'bg-[#B7131A]';
                        
                        $attempt = Auth::check() ? $quiz->attempts->first() : null;
                        $isCompleted = $attempt && $attempt->status === 'completed';
                        
                        // Kuis akan terkunci jika urutannya lebih besar atau sama dengan jumlah materi yang diselesaikan
                        $completedCount = $completedLessonsCount ?? 0;
                        $isLocked = ($index >= $completedCount); 
                        
                        $escapedTitle = htmlspecialchars(addslashes($quiz->title));
                        $escapedLevel = addslashes($quiz->level ?? 'Easy');
                        $questionCount = $quiz->question_count ?? 10;
                        $zepLink = $quiz->zep_link ?? 'https://zep.us/';
                    @endphp

                    <div @click="if(!{{ $isLocked ? 'true' : 'false' }}) { quizId = {{ $quiz->id }}; quizTitle = '{{ $escapedTitle }}'; quizLevel = '{{ $escapedLevel }}'; quizQuestions = {{ $questionCount }}; zepLink = '{{ $zepLink }}'; isStarted = false; showModal = true }" class="bg-white border border-[#B8B8B8] rounded-[15px] shadow-sm p-6 flex flex-col justify-between min-h-[240px] {{ $isLocked ? 'opacity-80' : 'cursor-pointer hover:-translate-y-1 transition-transform' }}">
                        <div>
                            <div class="flex items-center gap-4 mb-4">
                                <div class="w-12 h-12 rounded-[12px] {{ $iconBg }} flex items-center justify-center text-white flex-shrink-0">
                                    @if($isLocked)
                                        <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C9.243 2 7 4.243 7 7v3H6a2 2 0 00-2 2v8a2 2 0 002 2h12a2 2 0 002-2v-8a2 2 0 00-2-2h-1V7c0-2.757-2.243-5-5-5zM9 7c0-1.654 1.346-3 3-3s3 1.346 3 3v3H9V7zm7 13H8v-6h8v6z"/></svg>
                                    @else
                                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>
                                    @endif
                                </div>
                                <h3 class="text-[17px] font-semibold text-black leading-tight">{{ $quiz->title }}</h3>
                            </div>
                            <div class="flex items-center gap-3 mb-6">
                                <span class="px-4 py-1 {{ $levelBg }} {{ $levelText }} rounded-full text-[11px] font-bold tracking-wide">{{ $quiz->level ?? 'Easy' }}</span>
                                <div class="flex items-center gap-1.5 px-4 py-1 bg-[#F5F5F5] text-[#7B7675] rounded-full">
                                    <span class="w-[6px] h-[6px] bg-[#A5A5A5] rounded-full"></span>
                                    <span class="text-[11px] font-bold">{{ $questionCount }} Soal</span>
                                </div>
                            </div>
                        </div>
                        <div class="flex flex-col gap-2 mt-auto">
                            @if($isLocked)
                                <div class="w-full bg-[#B7131A] text-white text-[12px] font-medium py-2 px-4 rounded-[10px] text-center flex items-center justify-center gap-2">
                                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C9.243 2 7 4.243 7 7v3H6a2 2 0 00-2 2v8a2 2 0 002 2h12a2 2 0 002-2v-8a2 2 0 00-2-2h-1V7c0-2.757-2.243-5-5-5zM9 7c0-1.654 1.346-3 3-3s3 1.346 3 3v3H9V7zm7 13H8v-6h8v6z"/></svg>
                                    Terkunci
                                </div>
                            @elseif($isCompleted)
                                <div class="w-full bg-[#E3888C] text-white text-[12px] font-medium py-2 px-4 rounded-full text-center">
                                    ✓ Selesai
                                </div>
                                <div class="w-full bg-[#FBDAD9] text-[#E3888C] text-[12px] font-medium py-2 px-4 rounded-full text-center flex items-center justify-between shadow-sm cursor-pointer hover:bg-pink-200 transition">
                                    Ulangi Quiz <span class="text-lg font-bold leading-none">&rsaquo;</span>
                                </div>
                            @else
                                <div class="w-full bg-[#B7131A] text-white text-[12px] font-medium py-2 px-4 rounded-full text-center flex items-center justify-center gap-2 cursor-pointer hover:bg-[#8A070D] transition">
            ...
            <div x-show="showModal" style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm px-4">
                ...
                <div @click.outside="showModal = false" class="bg-white rounded-[15px] shadow-2xl w-full max-w-[850px] flex flex-col overflow-hidden relative" x-transition.opacity>
                                    ...
                                    <div class="border border-[#B7131A] rounded-[8px] p-4 bg-white shadow-sm flex flex-col">
                                        ...
                                        <div class="space-y-3 text-[12px]">
                                            ...
                                            <div class="flex items-center justify-between border-b border-gray-100 pb-2">
                                                ...
                                                <span class="font-bold text-black" x-text="quizQuestions + ' SOAL'">10 SOAL</span>
                                            </div>
                            ...
                            <div class="w-full md:w-[45%]">
                                <div class="border border-[#B8B8B8] rounded-[10px] p-6 flex flex-col items-center text-center justify-center bg-[#FAFAFA] h-full shadow-sm">
                                    ...
                                    <div class="w-[180px] h-[180px] bg-white border border-[#B7131A] rounded-[12px] p-2 flex items-center justify-center mb-5 relative">
                                        <!-- QR Code Placeholder -->
                                        <img :src="'https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=' + encodeURIComponent(zepLink)" alt="QR Code" class="w-full h-full object-contain">
                                    </div>
                                    ...
                                    <button @click="startQuiz()" class="text-[13px] text-[#B8B8B8] font-medium hover:text-[#B7131A] transition focus:outline-none">Lanjut di Perangkat Ini</button>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="px-8 py-5 border-t border-[#B8B8B8] flex flex-col md:flex-row items-center justify-between gap-4 bg-white">
                        ...
                        <button x-show="!isStarted" @click="startQuiz()" class="px-8 py-2.5 bg-[#B7131A] text-white rounded-[8px] text-[13px] font-bold hover:bg-[#8A070D] transition shadow-sm whitespace-nowrap focus:outline-none">
                            MULAI QUIZ
                        </button>
                        <button x-show="isStarted" @click="completeQuiz()" style="display: none;" class="px-8 py-2.5 bg-[#16A34A] text-white rounded-[8px] text-[13px] font-bold hover:bg-[#15803d] transition shadow-sm whitespace-nowrap focus:outline-none flex items-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                            SAYA SUDAH MENYELESAIKAN KUIS INI
                        </button>
                    </div>
                </div>
            </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
                ...
                <div class="flex flex-col">
                    ...
                    <a href="https://discord.gg/example" target="_blank" class="inline-flex items-center gap-3 bg-white border border-[#B8B8B8] rounded-full pl-1 pr-6 py-1 mb-6 hover:bg-gray-50 transition self-start shadow-sm">
                    ...
                    <a href="mailto:sisobatjrkm@gmail.com" class="inline-flex items-center gap-3 bg-white border border-[#B8B8B8] rounded-full pl-1 pr-6 py-1 hover:bg-gray-50 transition self-start shadow-sm">
